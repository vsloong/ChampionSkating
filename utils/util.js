...
/**
 * 存储所有动作等级的基础信息，方便下一次修改
 */
function setGradesStorage() {
  try {
    wx.setStorageSync(getApp().data.key_grades, getApp().grades)
    console.log("首次添加所有数据成功")
  } catch (e) {
    console.log("首次添加所有数据失败")
  }
}

/**
 * 存储某一动作等级的基础信息，方便下一次修改
 */
function setGradeStorage(gradeIndex) {
  try {
    wx.setStorageSync(getApp().data.key_grade + gradeIndex, getApp().grades[gradeIndex])
    console.log("首次添加grade" + gradeIndex + "数据成功")
  } catch (e) {
    console.log("首次添加grade" + gradeIndex + "数据失败")
  }
}
...
/**
 * 更新动作信息，以每一个等级为一个数据库，尽量减少内存的支出
 */
function updateFigureInfo(gradeIndex, figureIndex, figure) {
  //如果没有缓存信息那么先去缓存数据，然后再取出修改
  if (!isStorageSetted(getApp().data.key_grade + gradeIndex)) {
    setGradeStorage(gradeIndex)
  }

  wx.getStorage({
    key: getApp().data.key_grade + gradeIndex,
    //已经保存过缓存的数据了
    success: function (res) {
      var grade = res.data
      grade.figures[figureIndex].version = figure.version
      grade.figures[figureIndex].skill = figure.skill
      grade.figures[figureIndex].attention = figure.attention

      wx.setStorage({
        key: getApp().data.key_grade + gradeIndex,
        data: grade,
        success: function () {
          console.log("更新grade" + gradeIndex + "动作信息数据成功：" + JSON.stringify(grade.figures[figureIndex]))

        },
        fail: function () {
          console.log("更新grade" + gradeIndex + "动作信息数据失败！")
        }
      })
    },
    //之前还未保存过缓存数据
...
function setContactInfo(contactInfo) {
  wx.setStorage({
    key: getApp().data.key_contact,
    data: contactInfo,
    success: function () {
      console.log("添加联系方式成功")
    },
    fail: function () {
      console.log("添加联系方式失败")
    }
  })
}
...
function getContactInfo() {
  try {
    var value = wx.getStorageSync(getApp().data.key_contact)
    return value
  } catch (e) {
    return false
  }
...
function removeContactInfo() {
  wx.removeStorage({
    key: getApp().data.key_contact,
    success: function (res) {
      console.log("删除联系方式成功")
    },
    fail: function () {
      console.log("删除联系方式失败")
    }
  })
}
